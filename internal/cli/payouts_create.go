...
func init() {
	registerGeneratedEndpoint("payouts.create", newPayoutsCreateCmd)
	registerSurface(surface.Surface{
		...
		Flags: []surface.Flag{
			...
			},
			{
				...
				Description: "Whether to place the payout on hold automatically after creation.",
			},
			{
				...
				Kind:        surface.KindString,
				Description: "Reason for placing the payout on hold automatically.",
			},
			{
			...
			},
			{
				...
				Kind:        surface.KindString,
				Required:    true,
				Description: "Your unique identifier for the payout.",
			},
			{
				Name:        "metadata",
				In:          surface.InBody,
			...
			},
			{
				...
				Kind:        surface.KindString,
				Required:    true,
				Description: "Date when Straddle submits the payout for processing.",
				Format:      "date",
			},
		},
		HasBody:              true,
...
func newPayoutsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  straddle payouts create",
		Annotations: map[string]string{
			"straddle:endpoint":     "payouts.create",
			"straddle:operation-id": "createPayout",
	...
	applyOverlay("payouts.create", cmd)
	return cmd
}
