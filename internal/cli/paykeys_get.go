...
func init() {
	registerGeneratedEndpoint("paykeys.get", newPaykeysGetCmd)
	registerSurface(surface.Surface{
		Endpoint:    "paykeys.get",
...
func newPaykeysGetCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "get <id>",
		Short:   "Get a paykey",
		Example: "  straddle paykeys get <id>",
		Annotations: map[string]string{
			"straddle:endpoint":     "paykeys.get",
			"straddle:operation-id": "getPaykey",
			"straddle:method":       "GET",
			"straddle:path":         "/v1/paykeys/{id}",
			"mcp:read-only":         "true",
		},
	}
