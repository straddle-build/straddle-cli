...
func init() {
	registerGeneratedEndpoint("payouts.cancel", newPayoutsCancelGeneratedCmd)
...
func newPayoutsCancelGeneratedCmd(flags *rootFlags) *cobra.Command {
	s := surface.Surface{
		Endpoint:    "payouts.cancel",
		OperationID: "cancelPayout",
		Method:      "PUT",
	...
	cmd := &cobra.Command{
		Use:     "cancel <id>",
		Short:   "Cancel a payout",
		Example: "  straddle payouts cancel <id>",
		Annotations: map[string]string{
			"straddle:endpoint":     "payouts.cancel",
			"straddle:operation-id": "cancelPayout",
			"straddle:method":       "PUT",
			"straddle:path":         "/v1/payouts/{id}/cancel",
		},
	}
	...
	applyOverlay("payouts.cancel", cmd)
	return cmd
}
